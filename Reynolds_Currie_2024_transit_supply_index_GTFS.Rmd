...
AreaBn_calc_function <- function(areas_of_interest = areas_of_interest, areas_of_interest_id_field = areas_of_interest_id_field, gtfs_list_by_mode = gtfs_list_by_mode, EPSG_for_transform = EPSG_for_transform){
  ...
  for(mode_number in seq(1, length(gtfs_list_by_mode),1)){
    ...
    # update names of modes to match  
    names(Area_terms_for_each_area_of_interest_by_mode_by_stop) <- buffer_zone_definitions$short_name[1:length(Area_terms_for_each_area_of_interest_by_mode_by_stop)]
...
#EPSG_for_transform = 28355
#areas_of_interest_id_field = "sa1_code_2021"
#Area_terms_for_each_area_of_interest_by_mode_by_stop = Area_terms_for_each_area_of_interest_by_mode_by_stop
        ...
        if(nrow(gtfs_list_by_mode[[as.character(names(gtfs_list_by_mode[k]))]][["stops"]]) > 0) { 
         ...
         #which is calculated in the following for loop, which passes over all of the stops in the gtfs file for the mode in question
          for (i in seq(1, nrow(gtfs_list_by_mode[[ as.character(names(gtfs_list_by_mode[k]))]][["stops"]]), 1)) {
            ...
            stop_area_return_values_for_that_mode <- left_join(areas_of_interest, (Area_terms_for_each_area_of_interest_by_mode_by_stop[[k]][[j]] %>% as_tibble()))
            ...
            stop_areas_by_mode_k[,eval(names(gtfs_list_by_mode[k]))] <- stop_area_return_values_for_that_mode
         
        #close the if(no stops in gtfs for that mode) test 
            
        } else {#Add a NA record in the list of modes for modes that do not have any stops on that day
          stop_areas_by_mode_k[,eval(names(gtfs_list_by_mode[k]))] <- NA}
       
        
        stop_areas_by_day_j <- append(stop_areas_by_mode, list(stop_areas_by_mode_k), after = length(stop_areas_by_mode))
  ...
  for (j in seq(1, period_in_days, 1)) {
    ...
    ##for loop that drives the second level of the list, recording SI by mode
      for (k in seq(1, length(names(gtfs_list_by_mode)), 1)){   
        ...
        buffer_distance <- buffer_distance_listing[k,"distance"]
...
names(SI_for_day_of_analysis_by_mode_k) <- c( names(SI_for_day_of_analysis_by_mode_k[,1:ncol(SI_for_day_of_analysis_by_mode_k)-1]), eval(names(gtfs_list_by_mode[k])))                         
        ...
        if(nrow(gtfs_list_by_mode[[as.character(names(gtfs_list_by_mode[k]))]][["stops"]]) > 0) { 
         ...
         #which is calculated in the following for loop, which passes over all of the stops in the gtfs file for the mode in question
          for (i in seq(1, nrow(gtfs_list_by_mode[[ as.character(names(gtfs_list_by_mode[k]))]][["stops"]]), 1)) {
            Area_terms_for_stop_i <- (Area_terms_for_each_area_of_interest_by_mode_by_stop[[k]][[i]] %>% as_tibble())
            ...
            Area_terms_for_stop_i$add_to_SI <- as.numeric(Area_terms_for_stop_i$area_bn / Area_terms_for_stop_i$Area_area * Area_terms_for_stop_i$SL_Bn)
...
The Australian Bureau of Statistics (ABS) provides a range of shape files and other resources. This study made use of the absmapsdata R package [@R-absmapsdata] to access the 2021 Local Government Area (LGA) boundaries for the Greater Melbourne area.  The EPSG:28355 transform [@EPSG_28355] was used to shift longitude and latitude into metres, as per the Geocentric Datum of Australia 1994 (GDA95 / MGA zone 55) coordinates. 
