...
AreaBn_calc_function <- function(areas_of_interest = areas_of_interest, areas_of_interest_id_field = areas_of_interest_id_field, gtfs_list_by_mode = gtfs_list_by_mode, EPSG_for_transform = EPSG_for_transform){
  ...
  for(mode_number in seq(1, nrow(buffer_distance_listing),1)){
  ...
  <- int_circles
     ...
     # add element to the list for that mode
    Area_terms_for_each_area_of_interest_by_mode_by_stop[[buffer_zone_definitions$short_name[mode_number]]] <- Area_terms_for_each_area_of_interest_by_stop
    }
   
  # update names of modes to match  
        names(Area_terms_for_each_area_of_interest_by_mode_by_stop) <- buffer_distance_listing$mode
  
  
   
    LRT = tibble(stop_id = gtfs_list_by_mode$LRT$stops$stop_id[1], area_of_interest = areas_of_interest$sa1_code_2021[1], Area_Bn = 0))
  
  
  , 
 
  for(areas in seq(1, nrow(areas_of_interest), 1)){ 
    Area_terms_for_each_area_of_interest_by_mode_by_stop <- list(
    LRT = tibble(stop_id = gtfs_list_by_mode$LRT$stops$stop_id), 
    subway = tibble(stop_id = gtfs_list_by_mode$subway$stops$stop_id),
    rail = tibble(stop_id = gtfs_list_by_mode$rail$stops$stop_id),
    bus = tibble(stop_id = gtfs_list_by_mode$bus$stops$stop_id),
    ferry = tibble(stop_id = gtfs_list_by_mode$ferry$stops$stop_id), 
    cable_tram = tibble(stop_id = gtfs_list_by_mode$cable_tram$stops$stop_id), 
    aerial_lift = tibble(stop_id = gtfs_list_by_mode$aerial_lift$stops$stop_id), 
    funicular = tibble(stop_id = gtfs_list_by_mode$funicular$stops$stop_id), 
    trolleybus = tibble(stop_id = gtfs_list_by_mode$trolleybus$stops$stop_id),
    monorail = tibble(stop_id = gtfs_list_by_mode$monorail$stops$stop_id) 
    )
  }
    
    Area_terms_for_each_area_of_interest_by_mode_by_stop$LRT$Area_Bn <- tibble(
      sa1_code_2021 = st_drop_geometry(
        areas_of_interest[, areas_of_interest_id_field]),
      Area_Bn = 0
    ) 
  ...
  areas_of_interest_sf <- areas_of_interest_sf %>% 
    dplyr::mutate(Area_area = st_area(areas_of_interest_sf))
  # Add Area_Bn column for each mode to areas_of_interest_sf

  Area_terms_for_each_area_of_interest_by_mode_by_stop$LRT$areas_of_interest_id_field
  
  
#look up the buffer distances for each of the modes (m)
  buffer_distance_listing <- tibble(mode = names(gtfs_list_by_mode))
  buffer_distance_listing$distance <- NA
    for (m in seq(1, length(names(gtfs_list_by_mode)), 1)){   
          buffer_distance_listing[m,"distance"] <- buffer_zone_definitions %>% filter(short_name == as.character(names(gtfs_list_by_mode[m]))) %>% select(buffer) %>% as.numeric()
    }
  
#calculate the Area_Bn value, the Area_Area and the Area_Bn / Area_Area values for all stops in the gtfs file, and return as a list by day_of_analysis(j), each element of which contains a list by mode(k), each element is a stop (i). This involves 2 levels of for loops(day and mode), 1 level of if (does the mode for the day in question have any stops?), and 1 final level for loop (passing across all the stops for the mode and day in question)
  
  ###Initialise final reporting list (days) of lists (modes) of list (stops) of dataframe (area_code, area_bn, area_area, area_bn_divided_area_area)
  area_bn_divided_by_area_area_list_by_date_and_mode <- list()
      ...
      for (k in seq(1, length(names(gtfs_list_by_mode)), 1)){  
        ...
        if(nrow(gtfs_list_by_mode[[as.character(names(gtfs_list_by_mode[k]))]][["stops"]]) > 0) { 
         stop_area_return_values_for_that_mode <- tibble(area_bn = rep(0,nrow(areas_of_interest)), 
                                                         ...
                                                         area_bn_divided_by_area_area = rep(0,nrow(areas_of_interest)),
                                                         ) 
         #which is calculated in the following for loop, which passes over all of the stops in the gtfs file for the mode in question
          for (i in seq(1, nrow(gtfs_list_by_mode[[ as.character(names(gtfs_list_by_mode[k]))]][["stops"]]), 1)) {
           ...
           # and then transform to be expressed as per the EPSG that is being used for the analysis (ie. shift to metres)
            ...
            stop_area_return_values_for_that_mode[i,'area_bn'] <- int_circles$area_bn
            stop_area_return_values_for_that_mode[i,'area_area'] <- areas_of_interest_sf$Area_area
            stop_area_return_values_for_that_mode[i,'area_bn_divided_by_area_area'] <- 
              stop_area_return_values_for_that_mode$area_bn / stop_area_return_values_for_that_mode$area_bn_divided_by_area_area
       ...
       #close for (k) loop for calculating area_bn_divided_by_area_area for a single mode on a single day
      }  
       
      #add new element to the list_by_date for the date_of_analysis
      area_bn_divided_by_area_area_list_by_date_and_mode <- append(area_bn_divided_by_area_area_list_by_date_and_mode, list(stop_areas_by_day_j), after = length(area_bn_divided_by_area_area_list_by_date_and_mode))
    
      #update name of new element to match date
      names(area_bn_divided_by_area_area_list_by_date_and_mode) <- c(names(area_bn_divided_by_area_area_list_by_date_and_mode)[1:length(names(area_bn_divided_by_area_area_list_by_date_and_mode))-1], as.character(day_of_analysis))
