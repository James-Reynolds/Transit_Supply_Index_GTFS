...
#load pre-calculated df by mode 
victoria_2022_SI_df_by_mode <- read.csv2("results/Victoria_2022_SI_df_by_mode_SA12021_220814.csv")
victoria_2022_SI_df_by_mode$sa1_code_2021 <- as.character(victoria_2022_SI_df_by_mode$sa1_code_2021)
#add total column
...
kbl(victoria_2023_SI_df_by_mode %>% filter(sa1_code_2021 == "20403106915") %>% adorn_totals(where = "col"), caption = "Developed code output for SA1 20403106915, August 8, 2023")
...
kbl(victoria_2018_SI_df_by_mode %>% filter(sa1_code_2021 == "20403106915") %>% adorn_totals(where = "col"), caption = "Developed code output for SA1 20403106915, August 14, 2018")


kbl(victoria_2019_SI_df_by_mode %>% filter(sa1_code_2021 == "20403106915") %>% adorn_totals(where = "col"), caption = "Developed code output for SA1 20403106915, August 13, 2019")


kbl(victoria_2020_SI_df_by_mode %>% filter(sa1_code_2021 == "20403106915") %>% adorn_totals(where = "col"), caption = "Developed code output for SA1 20403106915, August 11, 2020")

kbl(victoria_2021_SI_df_by_mode %>% filter(sa1_code_2021 == "20403106915") %>% adorn_totals(where = "col"), caption = "Developed code output for SA1 20403106915, August 10, 2021")

kbl(victoria_2022_SI_df_by_mode %>% filter(sa1_code_2021 == "20403106915") %>% adorn_totals(where = "col"), caption = "Developed code output for SA1 20403106915, August 14, 2022")


```


It appears that no services ran on the day examined in 2022. Unfortunately, it is not possible to check this directly using the PTV website, but the GTFS for 2022 can be loaded and queried to explore this issue and identify whether it is correct or not.  

```{r Running_Creek_SI_calc_SA1_2022, echo=FALSE, warning=FALSE, message=FALSE, fig.fullwidth = TRUE, fig.cap= "$SI_{LGA2021, 10/8/21}$", fig.show="hold", out.width="50%"}

#manual check 2022
##load victoria_2022 as per code above, which takes care of duplicated stops and the lack of a route colour field etc. 

#extract bus gtfs from victoria_2022_list_by_mode list of tidygtfses
victoria_2022_bus <- as_tidygtfs(victoria_2022_list_by_mode$bus)

#From https://r-transit.github.io/tidytransit/articles/timetable.html



# get the id of the first stop in the trip's stop sequence
first_stop_id <- victoria_2022_bus$stop_times %>% 
  group_by(trip_id) %>% 
  summarise(stop_id = stop_id[which.min(stop_sequence)])

# join with the stops table to get the stop_name
first_stop_names <- left_join(first_stop_id, victoria_2022_bus$stops, by="stop_id")

# rename the first stop_name as trip_origin
trip_origins <- first_stop_names %>% select(trip_id, trip_origin = stop_name)

# join the trip origins back onto the trips
victoria_2022_bus$trips <- left_join(victoria_2022_bus$trips, trip_origins, by = "trip_id")

victoria_2022_bus$trips %>%
  select(route_id, trip_origin) %>%
  head()


stop_ids <- victoria_2022_bus$stops %>% 
  filter(stop_name == "Running Creek Rd/Kiewa Valley Hwy (Running Creek)") %>% 
  select(stop_id)

departures <- stop_ids %>% 
  inner_join(victoria_2022_bus$stop_times %>% 
               select(trip_id, arrival_time, 
                      departure_time, stop_id), 
             by = "stop_id")

departures <- departures %>% 
  left_join(victoria_2022_bus$trips %>% 
...
departures <- departures %>% 
  left_join(victoria_2022_bus$routes %>% 
              select(route_id, 
                     route_short_name), 
            by = "route_id")
...
services_on_220814 <- victoria_2022_bus$.$dates_services %>% 
  filter(date == "2022-08-14") %>% select(service_id)

departures_220814 <- departures %>% 
  inner_join(services_on_220814, by = "service_id")

##This returns an empty table, implying that the Albury to Mt Beauty services did not run on 2022-08-14



## Running for the following day as a check
services_on_220815 <- victoria_2022_bus$.$dates_services %>% 
  filter(date == "2022-08-15") %>% select(service_id)

departures_220815 <- departures %>% 
  inner_join(services_on_220815, by = "service_id")
### This returns a table with four rows, showing the typical service pattern for Wednesdays of 2 buses to Albury in the monring and two buses to My Beauty in the afternoon.

departures_220815 %>% 
  kable(caption = "Departures from 'Running Creek Rd/Kiewa Valley Hwy (Running Creek)' on 2022-08-15")



```

Querying the 2022 GTFS file for departures from 'Running Creek Rd/Kiewa Valley Hwy (Running Creek)' on 2022-08-14 returns no results, whereas querying for 2022-08-15 returns four services. This indicates that, for whatever reason, the services did not run in 2022 on the day in question^[Maybe it was a local school holiday, or similar.] used to calculated the SI scores, but they did run on the following day.  This explains why the $SI_{20403106915, 14/8/22}$ score is equal to zero, while SI scores for other years are not. 
