...
---
title: "Leveraging GTFS data to assess transit supply, and needs-gaps"
...
<!--Cross-case comparisons to Toronto, Canada, and Washington DC, USA, are also undertaken to test the results and gain understanding of how the Supply Index might be useful for practitioners, researchers and advocates. -->
...
### Spliting the GTFS file into individual modes
...
### Calculating the area-related terms of the SI equation
...
The area terms were calculated on a mode-by-mode and stop-by-stop basis. This involved first determining the amount of the catchment area (Area~Bn~) that falls into each geographical area of interest for each stop. This is then combined with the area for each geographical area of interest (Area~area~), to calculate the area-related terms in the SI equation. 

```{r Area_Bn_calc_as_functions, echo=FALSE, warning=FALSE, message=FALSE}


###Temporary variables to help with writing the function
#areas_of_interest <- sa12021 %>%
...
AreaBn_calc_function <- function(areas_of_interest = areas_of_interest, areas_of_interest_id_field = areas_of_interest_id_field, gtfs_list_by_mode = gtfs_list_by_mode, EPSG_for_transform = EPSG_for_transform){
  
   ##Initialise variables used in function
  Area_terms_for_each_area_of_interest_by_mode_by_stop <- list()
  
  ## Convert areas to sf format and project using EPSG transform
  ...
  ## 1st level for loop, looping over modes
  for(mode_number in seq(1, length(gtfs_list_by_mode),1)){
    
    ##Initialise variables used in second level of the for loop
    Area_terms_for_each_area_of_interest_by_stop <- list()
    Area_terms_for_each_area_of_interest_by_mode_by_stop <- append(Area_terms_for_each_area_of_interest_by_mode_by_stop, Area_terms_for_each_area_of_interest_by_stop)
    
    
    #look up buffer distance for the mode in question
    buffer_distance <- buffer_zone_definitions[[mode_number, 5]]
    
    #check if there are stops for the mode in question, if no stops return empty dataframe
    if (nrow(as.tibble(gtfs_list_by_mode[[buffer_zone_definitions$short_name[mode_number]]]["stops"])) == 0){
      Area_terms_for_each_area_of_interest_by_stop <- list()
    } else{
      ##2nd level for loop to loop over the stops
      for(stop_number in seq(1, nrow(as.tibble(gtfs_list_by_mode[[buffer_zone_definitions$short_name[mode_number]]]["stops"])),1)){
        #get stop_id
        stop_id_matching_stop_number <- ((gtfs_list_by_mode[[buffer_zone_definitions$short_name[mode_number]]]["stops"])$stops$stop_id)[stop_number]
        
        Area_terms_for_each_area_of_interest_by_stop <- append(Area_terms_for_each_area_of_interest_by_stop, stop_id_matching_stop_number)
        
        #create dataframe with the lat and lon value for the stop "stop_number"
        dat_sim <- data.frame(lat = gtfs_list_by_mode[[as.character(names(gtfs_list_by_mode[mode_number]))]][["stops"]]$stop_lat[stop_number],
        long = gtfs_list_by_mode[[as.character(names(gtfs_list_by_mode[mode_number]))]][["stops"]]$stop_lon[stop_number])
        ...
        # make a circle with the buffer distance as the radius around stop "stop_number"    
        ...
        int_circles <- st_intersection(areas_of_interest_sf, dat_circles)
            ...
            #drop_geometry and unneeded columns
            int_circles <- st_drop_geometry(int_circles) %>% 
              select(areas_of_interest_id_field, "Area_Bn", "Area_area")
            
            #calculate Area Bn divided by Area_area term 
            int_circles$Area_proportion <- int_circles$Area_Bn / int_circles$Area_area
          
            #drop calculated values back in the function return list
            
            Area_terms_for_each_area_of_interest_by_mode_by_stop[[buffer_zone_definitions$short_name[mode_number]]] <-
              append(Area_terms_for_each_area_of_interest_by_mode_by_stop[[buffer_zone_definitions$short_name[mode_number]]],
                   list(int_circles))
            
            #update the names of the stops entries to match stop_id field
            names(Area_terms_for_each_area_of_interest_by_mode_by_stop[[buffer_zone_definitions$short_name[mode_number]]]) <- 
              ((gtfs_list_by_mode[[buffer_zone_definitions$short_name[mode_number]]]["stops"])$stops$stop_id)[1:stop_number]
              
            #close the 2nd level for loop (looping over stops)
            }
          #close the else check for whether there are >0 stops for that mode
              }
      #close the 1st level for loop (looping over modes)
      }
  
    return(Area_terms_for_each_area_of_interest_by_mode_by_stop)      
    #close the function  
    }
  

```
As part of the development of the $Area_{Bn}$ code, the calculated 400 metre buffer area around the tram stop 44-Duncraig Ave/Wattletree Rd (Armadale) (stop_ID 10371) was maped^[For those of you reading the rmarkdown file, the code used to produce the map is commented in the above chunk as ##TEST CODE Map to check that it is working the way it is expect to.]. Figure \ref{fig:test_10371} shows the tested stop location, the SA12021 boundaries nearby, and the map output by the code developed here.  
```{r test_10371, echo=FALSE, fig.fullwidth = TRUE, fig.show="hold", out.width='33%', fig.cap='Code test to map tram stop at Duncraig Ave/Wattletree Rd: Google maps location (left); ABS 2021 SA1 boundaries (centre); output map of buffer zone overlayed on SA1 2021 boundaries for AreaBn calculation.'}
#knitr::include_graphics("images/Google_maps_around_10371.png")
#knitr::include_graphics("images/SA1_2021_around_10371_ABS.png")
#knitr::include_graphics("SA1_2021_10371_map_output")
```

In Figure \ref{fig:test_10371}(left) the location of the tram stop ID 10371 is shown, together with a 400m distance reaching to the intersection of Glenferrie Road and Union Street. In the centre image the ABS' map of the SA12021 boundaries are shown around the stop (indicated in green), which indicates that the stop is located within a triangular SA1 zone, which extends north and east to the intersection with Glenferrie Road and Union Street.  The right image shows the output generated by the test code, indicating the spatial areas within each of the surrounding SA12021 zones that are within the 400 metre catchment radius of the stop. This image similarly indicates the green zone (SA1 20606113507) as a triangular zone extending to the north-east of the buffer zone around the stop. 


### Combining all the terms

The final step is to combined all the terms calculated above^[For each stop the area of the buffer zone that falls into each of the areas of interest ($Area_{Bn}), divided by the area for each geographical area of interest (Area~area~), and multiply by the number of stop arrivals (SL~Bn~)] to calculate the contribution to the SI scores made by just that single stop for every area of interest. These are then added to a cumulative total field for each area of interest, and the calculations are repeated until all stops and modes in the GTFS file have been included. 
...
#mel3 <- read_gtfs("data/3google_transit.zip")
#mel3_list_by_mode <- split_gtfs_into_modes_and_put_in_list(mel3)
gtfs_list_by_mode <- mornington_2018_list_by_mode
#start_date = "2023-04-27"
#period_in_days = 2
#EPSG_for_transform = 28355
#areas_of_interest_id_field = "sa1_code_2021"
Area_terms_for_each_area_of_interest_by_mode_by_stop = AreaBn_calc_function_output_2021 


#### The function ________
SI_calc_function <- function(areas_of_interest = areas_of_interest, areas_of_interest_id_field = areas_of_interest_id_field, gtfs_list_by_mode = gtfs_list_by_mode, start_date = start_date, period_in_days = period_in_days, EPSG_for_transform = EPSG_for_transform, Area_terms_for_each_area_of_interest_by_mode_by_stop=Area_terms_for_each_area_of_interest_by_mode_by_stop){
  ...
  #Add a SI column to the areas_of_interest to store the SI values
  areas_of_interest <- areas_of_interest %>% 
    dplyr::mutate(SI = 0)
    
#obtain the precalculated Area_Bn / Area_Area values for all stops in the gtfs file, and return as a list by day_of_analysis(j), each element of which contains a list by mode(k), each element is a stop (i). This involves 2 levels of for loops(day and mode), 1 level of if (does the mode for the day in question have any stops?), and 1 final level for loop (passing across all the stops for the mode and day in question)
  
  ###Initialise final reporting list (days) of lists (modes) of list (stops) of dataframe (area_code, area_bn, area_area, area_bn_divided_area_area --- this bit is what is precalculated in the AreaBn_calc_function above)
  Area_proportion_list_by_date_and_mode <- list()
  ...
  for (j in seq(1, period_in_days, 1)) {
     ...
     stop_areas_by_day_j <- list()
      for (k in seq(1, length(names(gtfs_list_by_mode)), 1)){  
        ##Initialise variables used in 2nd level of the list 
          stop_areas_by_mode_k <- list()
        ...
        if(nrow(gtfs_list_by_mode[[as.character(names(gtfs_list_by_mode[k]))]][["stops"]]) > 0) { 
         stop_area_return_values_for_that_mode <- tibble(Area_Bn = rep(0,nrow(areas_of_interest)), 
                                                         Area_area = rep(0,nrow(areas_of_interest)),
                                                         Area_proportion = rep(0,nrow(areas_of_interest)),
                                                         ) 
         #which is calculated in the following for loop, which passes over all of the stops in the gtfs file for the mode in question
          for (i in seq(1, nrow(gtfs_list_by_mode[[ as.character(names(gtfs_list_by_mode[k]))]][["stops"]]), 1)) {
            ##Initialise variables used in 3rd level (list of stops)
            stop_area_return_values_for_that_mode <- list()  
            
            ###join the pre-calculated Area_terms_for_each_area_of_interest_by_mode_by_stop for this mode (k) and stop (stop i) to the list of areas_of_interest
            
            #first extract the values for this stop
            For_join_Area_terms_for_each_area_of_interest_by_mode_by_stop <- (Area_terms_for_each_area_of_interest_by_mode_by_stop[[                                                                                             names(gtfs_list_by_mode[k])]][names(Area_terms_for_each_area_of_interest_by_mode_by_stop[[names(gtfs_list_by_mode[k])]])[i]])
            
            #then turn the extract list (with one element, a dataframe) into just a dataframe
            For_join_Area_terms_for_each_area_of_interest_by_mode_by_stop <- For_join_Area_terms_for_each_area_of_interest_by_mode_by_stop[[1]]
            
            #then join the data from back to our areas_of_interest 
            stop_area_return_values_for_that_mode <- left_join((st_drop_geometry(areas_of_interest)), 
For_join_Area_terms_for_each_area_of_interest_by_mode_by_stop)
            #zero out any NAs after that join
            stop_area_return_values_for_that_mode[is.na(stop_area_return_values_for_that_mode)] = 0
            
            
            #close (i) loop for obtaining the area values for for stop 
            }
        
            #add the areas for stop i to the mode-specific dataframe
            stop_areas_by_mode_k[names(gtfs_list_by_mode[k])] <- stop_area_return_values_for_that_mode
         
        #close the if(no stops in gtfs for that mode) test 
            
        } else {#Add an empty record in the list of modes for modes that do not have any stops on that day
          stop_areas_by_mode_k[names(gtfs_list_by_mode[k])] <- tibble()
        }
       
        
        stop_areas_by_day_j <- append(stop_areas_by_day_j, stop_areas_by_mode_k)
       #close for (k) loop for calculating Area_proportion for a single mode on a single day
      }  
       
      #add new element to the list_by_date for the date_of_analysis
      Area_proportion_list_by_date_and_mode <- append(Area_proportion_list_by_date_and_mode, list(stop_areas_by_day_j), after = length(Area_proportion_list_by_date_and_mode))
    
      #update name of new element to match date
      names(Area_proportion_list_by_date_and_mode) <- c(names(Area_proportion_list_by_date_and_mode)[1:length(names(Area_proportion_list_by_date_and_mode))-1], as.character(day_of_analysis))
        ...
        if(nrow(gtfs_list_by_mode[[as.character(names(gtfs_list_by_mode[k]))]][["stops"]]) > 0) { 
         ...
         #which is calculated in the following for loop, which passes over all of the stops in the gtfs file for the mode in question
          for (i in seq(1, nrow(gtfs_list_by_mode[[ as.character(names(gtfs_list_by_mode[k]))]][["stops"]]), 1)) {
                        ...
                        Area_terms_for_stop_i <-    (Area_terms_for_each_area_of_interest_by_mode_by_stop[[                                                                                             names(gtfs_list_by_mode[k])]][names(Area_terms_for_each_area_of_interest_by_mode_by_stop[[names(gtfs_list_by_mode[k])]])[i]])[[1]]
            ...
            Area_terms_for_stop_i$SL_Bn <- rep(
              ...
              nrow(Area_terms_for_stop_i))
    
            #Calculate SI for stop i and drop units
            Area_terms_for_stop_i$add_to_SI <- as.numeric(Area_terms_for_stop_i$Area_proportion * Area_terms_for_stop_i$SL_Bn)
  
            
            #Create ordinary tibble with Area of Interest identification code and SIs from stop i to add to the running totals
            export_to_SI_for_day_of_analysis_by_mode_k <- Area_terms_for_stop_i %>% select(c(as.character(areas_of_interest_id_field), add_to_SI)) 
...
To test the developed code results were generated for a two cases: the Mornington Railway on the Moornington Penninsula in Victoria, Australia; and the more general PTV feed that covers public transport across all of Victoria, Australia. These cases were selected primarily for convenience, as the authors are familiar with the Victorian transit network and so well placed to assess whether output results are correct.  Results were processed using the ggmaps [@R-ggmap], ggplot [@R-ggplot2], ggstatsplot [@R-ggstatsplot] and kable [@R-kableExtra; @R-knitr] packages, with data processing leveraging the tidyverse approach [@R-tidyverse].

The Australian Bureau of Statistics (ABS) provides a range of shape files and other resources. This study made use of the absmapsdata R package [@R-absmapsdata] to access the 2016 and 2021 SA1 (Statistical Area 1) and 2021 Local Government Area (LGA) boundaries for Victoria.  The EPSG:28355 transform [@EPSG_28355] was used to shift longitude and latitude into metres, as per the Geocentric Datum of Australia 1994 (GDA95 / MGA zone 55) coordinates. 
...
### Mornington Railway, Victoria, Australia

The Mornington Penninsula railway is a historic rail line, providing tourist and excursion services. GTFS feeds are available for this service for 2017 and 2018.  The developed code was tested on these GTFS feeds, which have the advantage of only having a small number of stops and services. The testing involved examination of the SI scores at the SA1 level for areas close to the railway. 


```{r Mornington_2018, echo=FALSE, warning=FALSE, message=FALSE}


mornington_2018 <- read_gtfs("data/Mornington_Railway/2018/20180109/gtfs.zip")

#deal with issue to do with missing route_color information
mornington_2018$routes$route_color <- "Blue"
...
mornington_2018$routes$route_type <- as.integer(2)


#identify duplicate stops
mornington_2018_duplicated_stops <- tabyl(mornington_2018$stops$stop_id) %>% filter (n>1)
names(mornington_2018_duplicated_stops) <- c("stop_id", "n", "percent")
mornington_2018_duplicated_stops <- left_join(mornington_2018_duplicated_stops, mornington_2018$stops)

##discard duplicates
mornington_2018$stops <- mornington_2018$stops[!duplicated(mornington_2018$stops$stop_id),]

##split modes
mornington_2018_list_by_mode <- split_gtfs_into_modes_and_put_in_list(mornington_2018)


##set inputs to SI_calc function
areas_of_interest <- sa12021 %>% filter(sa3_name_2021 == "Mornington Peninsula")
...
#run the Area_Bn_calc_function
AreaBn_calc_function_output_2021 <- AreaBn_calc_function(areas_of_interest = areas_of_interest, areas_of_interest_id_field = areas_of_interest_id_field, gtfs_list_by_mode = gtfs_list_by_mode, EPSG_for_transform = EPSG_for_transform)

##run SI_calc function
mornington_2018_SI_list_by_date_and_mode <- SI_calc_function(areas_of_interest =  areas_of_interest, areas_of_interest_id_field = areas_of_interest_id_field, gtfs_list_by_mode = mornington_2018_list_by_mode, start_date = start_date, period_in_days = period_in_days, EPSG_for_transform = EPSG_for_transform, Area_terms_for_each_area_of_interest_by_mode_by_stop = AreaBn_calc_function_output_2021)

#run aggregation function to create df by mode for all 7 days
improved_speed_mornington_2018_SI_df_by_mode <-  convert_SI_list_by_date_and_mode_to_SI_df_by_mode.function(mornington_2018_SI_list_by_date_and_mode)

#load pre-calculated df by mode (for just one day)
#mornington_2018_SI_df_by_mode <- read.csv("mornington_2018_SI_df_by_mode_20210810.csv")
#mornington_2018_SI_df_by_mode$lga_code_2021 <- as.character(mornington_2018_SI_df_by_mode$lga_code_2021)

#Join SI to map data
mornington_2018_SI_df_by_mode_sf <- left_join(mornington_2018_SI_df_by_mode, areas_of_interest)

```



```{r Mornington_2017, echo=FALSE, warning=FALSE, message=FALSE}


mornington_2017 <- read_gtfs("data/Mornington_Railway/2017/170309/gtfs.zip")

#deal with issue to do with missing route_color information
mornington_2017$routes$route_color <- "Blue"

#convert route type from 107 (tourist railway) to 2 (rail) ---NEW FEATURE NEEDED --- EXPAND HANDLING OF MODES TO ENABLE CALCULATION OF MODES FROM EXTENDED GTFS LIST
mornington_2017$routes$route_type <- as.integer(2)


#identify duplicate stops
mornington_2017_duplicated_stops <- tabyl(mornington_2017$stops$stop_id) %>% filter (n>1)
names(mornington_2017_duplicated_stops) <- c("stop_id", "n", "percent")
mornington_2017_duplicated_stops <- left_join(mornington_2017_duplicated_stops, mornington_2017$stops)

##discard duplicates
mornington_2017$stops <- mornington_2017$stops[!duplicated(mornington_2017$stops$stop_id),]

##split modes
mornington_2017_list_by_mode <- split_gtfs_into_modes_and_put_in_list(mornington_2017)
...
gtfs_list_by_mode = mornington_2017_list_by_mode
...
##run SI_calc function
mornington_2017_SI_list_by_date_and_mode <- SI_calc_function(areas_of_interest =  areas_of_interest, areas_of_interest_id_field = areas_of_interest_id_field, gtfs_list_by_mode = mornington_2017_list_by_mode, start_date = start_date, period_in_days = period_in_days, EPSG_for_transform = EPSG_for_transform, Area_terms_for_each_area_of_interest_by_mode_by_stop = AreaBn_calc_function_output_2021)

#run aggregation function to create df by mode for all 7 days
improved_speed_mornington_2017_SI_df_by_mode <-  convert_SI_list_by_date_and_mode_to_SI_df_by_mode.function(mornington_2017_SI_list_by_date_and_mode)

#load pre-calculated df by mode (for just one day)
#mornington_2017_SI_df_by_mode <- read.csv("mornington_2017_SI_df_by_mode_20210810.csv")
#mornington_2017_SI_df_by_mode$lga_code_2021 <- as.character(mornington_2017_SI_df_by_mode$lga_code_2021)

#Join SI to map data
mornington_2017_SI_df_by_mode_sf <- left_join(mornington_2017_SI_df_by_mode, areas_of_interest)
...
### Transit services across Victoria, Australia
...
The developed code was tested for both SA1- and LGA-level areas of interest. Output was obtained for SA1-level areas using a GTFS file from late April, 2023, running for a full 7-day period.  For the LGA-level output the results were compared between 2016 and 2021, to match the Australian census dates. The Australian census is undertaken in early August every 5 years. GTFS feeds were therefore selected for the first week of August of each year, with code output produced for only the day of the census itself^[It takes about a day of processing time to run the code for all of the stops in Vicotira for a single 'day' of service. Hence, only the census days (rather than weeks) were analysed to speed development.]. Minor corrections were made to the GTFS files to remove duplicate stop_ids^[These involved minor discrepancies in either the stop name, latitude or longitude.]. 
...
##set inputs to SI_calc function
areas_of_interest <- lga2021 %>% filter(state_name_2021 == "Victoria")
start_date = "2021-08-10"
period_in_days = 7
EPSG_for_transform = 28355
areas_of_interest_id_field = "lga_code_2021"
gtfs_list_by_mode = victoria_2021_list_by_mode
...
##run SI_calc function
victoria_2021_SI_list_by_date_and_mode <- SI_calc_function(areas_of_interest =  areas_of_interest, areas_of_interest_id_field = areas_of_interest_id_field, gtfs_list_by_mode = victoria_2021_list_by_mode, start_date = start_date, period_in_days = period_in_days, EPSG_for_transform = EPSG_for_transform, Area_terms_for_each_area_of_interest_by_mode_by_stop = AreaBn_calc_function_output_2021)

#run aggregation function to create df by mode for all 7 days
improved_speed_victoria_2021_SI_df_by_mode <-  convert_SI_list_by_date_and_mode_to_SI_df_by_mode.function(victoria_2021_SI_list_by_date_and_mode)
...
#deal with issue to do with missing route_color information
victoria_2016$routes$route_color <- "Blue"
...
#run the Area_Bn_calc_function
AreaBn_calc_function_output_2016 <- AreaBn_calc_function(areas_of_interest = areas_of_interest, areas_of_interest_id_field = areas_of_interest_id_field, gtfs_list_by_mode = gtfs_list_by_mode, EPSG_for_transform = EPSG_for_transform)

##run SI_calc function
victoria_2021_SI_list_by_date_and_mode <- SI_calc_function(areas_of_interest =  victoria_areas_of_interest, areas_of_interest_id_field = areas_of_interest_id_field, gtfs_list_by_mode = victoria_2021_list_by_mode, start_date = start_date, period_in_days = period_in_days, EPSG_for_transform = EPSG_for_transform, Area_terms_for_each_area_of_interest_by_mode_by_stop = AreaBn_calc_function_output_2016)
...
## Mornington Railway, Victoria, Australia: Supply Index results for SA1s


Figure \ref{fig:Mornington_SI_2018} show the $SI_{SA12021,9th-15thJan2018}$ values from the Mornington Railway service for SA1s within the SA3 region of the Mornington Peninsula, .



```{r Mornington_SI_2018, echo=FALSE, warning=FALSE, message=FALSE, fig.fullwidth = TRUE, fig.cap= "$SI_{SA12021,9th-15thJan2018}$"}
mornington_2018_SI_total <- mornington_2018_SI_df_by_mode %>% adorn_totals(where = "col")

mornington_2018_SI_total <- left_join(mornington_2018_SI_total, LGA_map_data)


map <- mornington_2018_SI_total %>%
  ggplot() +
  geom_sf(aes(geometry = geometry,  # use the geometry variable
