...
```{r Victoria_SI_changes, echo=FALSE, warning=FALSE, message=FALSE, fig.show="hold", out.width="50%", fig.fullwidth = TRUE, fig.cap= "Change in SI on census day 2021 compared to 2016, by LGA"}
...
victoria_2021_2016_SI_by_mode <- add_row(victoria_2021_2016_SI_by_mode, 
                                         pivot_longer(victoria_2021_SI_df_by_mode %>% select(!Total), cols = 2:11, names_to = "mode", values_to = "SI change") %>% add_column(.after = "lga_code_2021", Census = "2021"))  

victoria_2021_2016_SI_by_mode <- left_join(victoria_2021_2016_SI_by_mode, LGA_map_data)


ggwithinstats(
  data = victoria_2021_2016_SI_by_mode %>%
  filter(lga_name_2021 == "Melbourne") %>% 
    filter(`SI change` != 0), ## data frame from which variables are taken
  x = "Census", ## predictor/independent variable
  y =   "SI change", ## dependent variable
  pairwise.comparisons = TRUE,
  xlab = "Census", ## label for the x-axis
  ylab = "SI on census day, by mode", ## label for the y-axis
  caption = "Analysis using ggstatsplot package"
) + 
  scale_y_continuous(labels=percent)
