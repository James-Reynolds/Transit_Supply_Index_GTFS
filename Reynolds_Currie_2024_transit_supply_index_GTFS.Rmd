...
split_gtfs_into_modes_and_put_in_list <- function(gtfs){ 
  gtfs_LRT <- filter_by_route_type(gtfs, route_type = 0)
  gtfs_subway <- filter_by_route_type(gtfs, route_type = 1)
  gtfs_rail <- filter_by_route_type(gtfs, route_type = 2)
  gtfs_bus <- filter_by_route_type(gtfs, route_type = 3)
  gtfs_ferry <- filter_by_route_type(gtfs, route_type = 4)
  gtfs_cable_tram <- filter_by_route_type(gtfs, route_type = 5)
  gtfs_aerial_lift <- filter_by_route_type(gtfs, route_type = 6)
  gtfs_funicular <- filter_by_route_type(gtfs, route_type = 7)
  gtfs_trolleybus <- filter_by_route_type(gtfs, route_type = 11)
  gtfs_monorail <- filter_by_route_type(gtfs, route_type = 12)
...
SI_calc_function <- function(areas_of_interest = areas_of_interest, gtfs_list_by_mode = gtfs_list_by_mode, start_date = start_date, period_in_days = period_in_days, EPSG_for_transform = EPSG_for_transform){
  ...
  for (j in seq(1, period_in_days-1, 1)) {
    ...
    #for loop that drives the second level of the list, recording SI by mode
      for (k in seq(1, length(names(gtfs_list_by_mode)), 1)){ 
        #look up the buffer distance for that mode 
        ...
        #check if that mode is empty
        ifelse(is.na(nrow(
         gtfs_list_by_mode[[
           as.character(names(gtfs_list_by_mode[k]))]]
         ["stops"]
         ), 
         #if there are no stops in the GTFS for that mode, then return vector of zeros (one for each area of interest) to the 2nd level list, which will be appended and named lower down. 
         SI_return_value_for_that_mode <- rep(0,nrow(areas_of_interest)),
         #otherwise return the SI for the mode and day in question
    ...
    # Convert the dataframe (with the stop latitude and longitude) into a sf object, with the crs set to EPSG:4326 (which is the CRS for lat/long values), 
   ...
   # lookup buffer distance for mode k  
    buffer_distance <- pull(buffer_zone_definitions[buffer_zone_definitions$short_name == as.character(names(gtfs_list_by_mode[k])),"buffer"])
   # Buffer circles by the buffer distance
    ...
    int_circles <- st_intersection(areas_of_interest_sf, dat_circles)
              ...
              ##Map to check that it is working the way it is expect to.
             # map <- int_circles %>%
            #  ggplot() +
             # geom_sf(aes(geometry = geometry,  # use the geometry variable
              #  fill = sa1_code_2021),     # fill by SA1_code
               # lwd = 0,                  # remove borders
              #  show.legend = TRUE) +    # keep legend
              #theme_void() +                    # clears other plot elements
              #coord_sf()
    ...
    gtfs_k <- eval(parse(text = paste("gtfs_list_by_mode$",names(gtfs_list_by_mode)[k],sep = "")))
    # then running the arrivals timetable function for the stop and day. 
      int_circles$SL_Bn <- arrival_timetable_function(gtfs = gtfs_k, stop_to_show = gtfs_list_by_mode[[
    as.character(names(gtfs_list_by_mode[k]))]]$stops$stop_name[i], date_to_show = as.Date(day_of_analysis))
    
 
    
    #Calculate SI for stop i
    #Note the 1,000,000 factor to convert square km to square m
    int_circles$SI <- int_circles$area_bn / int_circles$SL_Bn
  
    #Create ordinary tibble with SA1 code and SIs from stop i to add to the running totals
    export_to_ccds <- tibble(sa1_code_2021 = int_circles$sa1_code_2021, add_to_SI = int_circles$SI)

   #drop add_SI column from SA1 map
    areas_of_interest_sf <- areas_of_interest_sf[, !(colnames(areas_of_interest_sf) %in% "add_to_SI")]
  
  
    #merge based on sa1_code_2021
    areas_of_interest_sf <- left_join(areas_of_interest_sf, export_to_ccds)
  
    #SI and add_to_SI to non-unit numbers
    areas_of_interest_sf$SI <- as.vector(areas_of_interest_sf$SI)
    areas_of_interest_sf$add_to_SI <- as.vector(areas_of_interest_sf$add_to_SI)
  
    #replace NA with 0 
    areas_of_interest_sf[is.na(areas_of_interest_sf)] = 0
  
    #add the SIs for stop i to the running total of SI
    areas_of_interest_sf$SI <- areas_of_interest_sf$SI + areas_of_interest_sf$add_to_SI
 
    #show running total of stops examined so far
    i 
  }
  return(SI_return_value_for_that_mode)
          )
     
    
    #add new element for the list
    SI_list <- append(SI_list, )
    #update name of new element to match date
    names(SI_list) <- c(names(SI_list), as.character(day_of_analysis))    
