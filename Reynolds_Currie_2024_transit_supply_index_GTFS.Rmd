...
st_drop_geometry(sa1_map_data %>% filter(sa1_code_2021 == "20401105720"))
```

The area of the My Buller SA1 is `r st_drop_geometry(sa1_map_data %>% filter(sa1_code_2021 == "20401105720"))$areasqkm_2021 %>% format(digits = 3)`km^2^.  By inspection, the entire 400m radius catchment area of the bus stop lies entirely within the SA1 20401105720 boundaries^{Although it appears that the bus stop's location has been incorrectly geocoded to be some distance south of any actual roads.].

![Mt Buller Visitor Centre bus stop location, source Google Maps](images/Mt_buller_bus_stop.png)

Hence the $Area_{Bn}/ Area_{SA1_Area} $ term for this bus stop is equal to $(\pi 400^2) / 829007400 =$ `r (pi * 400 * 400 / 22576200) %>% label_scientific(digits = 3)()`.  
...
Our purpose here, however, is to verify the calculation of the SI score from what is in the GTFS file, not what might be the real-world conditions. It is unclear why there is a disrepancy between the number of arrivals report in the GTFS versus the printed timetable, but for the purposes of this verfication calculation the SL~Bn~ term for one week (during the snow season) appears to be 8, giving a total $SI_{20401105720, one week}$ score of `r (8* pi * 400 * 400 / 22576200) %>% label_scientific(digits = 3)()`.  


Next the SI calculation code is run on the 4 August 2023 GTFS file, for the seven days starting August 7.  However, the area of interest is reduced so as to only include the SA1 zone numbered 20401105720. 
...
#set inputs to SI_calc function
victoria_areas_of_interest <- sa1_map_data %>% filter(sa1_code_2021 == "20401105720")
