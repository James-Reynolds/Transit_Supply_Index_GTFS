...
SI_calc_function <- function(areas_of_interest = areas_of_interest, areas_of_interest_id_field = areas_of_interest_id_field, gtfs_list_by_mode = gtfs_list_by_mode, start_date = start_date, period_in_days = period_in_days, EPSG_for_transform = EPSG_for_transform){
  ...
  #the top level list (of lists), with one element for each day of analysis 
  SI_list_by_date <- list()
...
#2-level list to put the SI results in, with each element representing one of the days that is being analysed. Each element will itself also be a list, with each element representing the SI results for one of the modes (see gtfs_list_by_mode names)
  
  
  
  #for loop that drives the first level of the list, recording the day of analysis
  for (j in seq(1, period_in_days-1, 1)) {
    ...
    #for loop that drives the second level of the list, recording SI by mode
      for (k in seq(1, length(names(gtfs_list_by_mode)), 1)){ 
        ...
        ##Initialise variables used in second level of the list
        # This becomes a list (of lists), with one element for each mode (k) for the day_of_analysis in question (j)
        SI_for_day_of_analysis_and_mode_k <- list()
        ...
        #check if the gtfs mode in question has any stops - many of the modes (e.g. Monorail) will typically be zero
        ifelse(is.na(nrow(gtfs_list_by_mode[[
           as.character(names(gtfs_list_by_mode[k]))]]
         ["stops"]
         )), 
         
         #if there are no stops in the GTFS for that mode, then append vector of zeros (one for each area of interest) to a new element the 2nd level list.
          SI_for_day_of_analysis_and_mode_k <- append(
            SI_for_day_of_analysis_and_mode_k, 
            rep(0,nrow(areas_of_interest), 
                x = length(SI_for_day_of_analysis_and_mode_k)
                )
            )
          # and change the name of the new element to mode k
          names(SI_list_by_date) <- c(names(SI_list_by_date), as.character(day_of_analysis)),
            
            #otherwise calculated and return the SI for the mode and day in question
         ...
         #which is calculated in the following for loop, which passes over all of the stops in the gtfs file for the mode in question
          for (i in seq(1, nrow(
            gtfs_list_by_mode[[
            as.character(names(gtfs_list_by_mode[k]))]][["stops"]]), 1) {
           ...
           # and then transform to be expressed as per the EPSG that is being used for the analysis (ie. shift to metres)
            ...
            int_circles$SI <- as.numeric(int_circles$area_bn / int_circles$Area_area * int_circles$SL_Bn)
  
            #Create ordinary tibble with Area of Intersect identification code and SIs from stop i to add to the running totals
            export_to_SI_for_day_of_analysis_and_mode_k <- tibble(eval(parse(text=areas_of_interest_id_field)) = int_circles$eval(parse(text=areas_of_interest_id_field)), add_to_SI = int_circles$SI)

            #drop add_SI column from dataframe reporting SI_by_mode scores up to the date loop (SI_for_day_of_analysis_and_mode_k)
            SI_for_day_of_analysis_and_mode_k <- SI_for_day_of_analysis_and_mode_k[, !(colnames(SI_for_day_of_analysis_and_mode_k) %in% "add_to_SI")]
  
  
            #merge based on eval(parse(text=areas_of_interest_id_field))
            SI_for_day_of_analysis_and_mode_k <- left_join(SI_for_day_of_analysis_and_mode_k, export_to_SI_for_day_of_analysis_and_mode_k)
  
            #convert add_to_SI to non-unit numbers
            SI_for_day_of_analysis_and_mode_k$add_to_SI <- as.vector(SI_for_day_of_analysis_and_mode_k$add_to_SI)
  
            #replace NA with 0 
            SI_for_day_of_analysis_and_mode_k[is.na(SI_for_day_of_analysis_and_mode_k)] = 0
  
            #add the SIs for stop i to the running total of SI
            SI_for_day_of_analysis_and_mode_k$SI <- SI_for_day_of_analysis_and_mode_k$SI + SI_for_day_of_analysis_and_mode_k$add_to_SI
          
            #close for loop for calculating SI for a single mode on a single day
            }
 
          #close ifelse test for whether there are any stops for mode k in the gtfs stops file
          )
        
        #add new element to the list for that mode
        SI_for_day_of_analysis_list_by_mode <- append(SI_for_day_of_analysis_list_by_mode, SI_for_day_of_analysis_and_mode_k$SI, after = length(SI_for_day_of_analysis_list_by_mode))
        #update name of new element to match date
        names(SI_list_by_date) <- c(names(SI_list_by_date), as.character(day_of_analysis))

      #close for loop for calculating a list of SIs by mode for a single day
      }
    
    #add new element for the list for the date_of_analysis
    SI_list_by_date <- append(SI_list_by_date, SI_list_for_day_of_analysis, after = length(SI_list_by_date))
    #update name of new element to match date
    names(SI_list_by_date) <- c(names(SI_list_by_date), as.character(day_of_analysis))

    #close for loop for calculating a list of days, with each day containing a list of SIs by mode
    }        

    areas_of_interest_sf <- areas_of_interest_sf %>% dplyr::mutate(SI = SI_list_by_date)    
 return(areas_of_interest_sf)
...
#mel1_SICCD <- SICCD_calc_function(areas_of_interest = areas_of_interest, gtfs_list_by_mode = mel1, start_date = start_date, EPSG_for_transform = EPSG_for_transform, buffer_distance = 800)
