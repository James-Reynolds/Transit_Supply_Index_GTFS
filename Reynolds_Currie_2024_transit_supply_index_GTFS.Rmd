...
split_gtfs_into_modes_and_put_in_list <- function(gtfs){ 
  gtfs_LTR <- filter_by_route_type(gtfs, route_type = 0)
...
#for loop that drives the first level of the list, recording the day of analysis
  for (j in seq(1, period_in_days-1)) {
    ...
    SI_list <- append(SI_list, list())
    #update name of new element to match date
    names(SI_list) <- c(names(SI_list), as.character(day_of_analysis))
      for (k in seq(1:length(names(gtfs_list_by_mode))){ 

  for (i in seq(1,nrow(gtfs_list_by_mode$stops),1)) {
      dat_sim <- data.frame(lat = gtfs_list_by_mode$stops$stop_lat[i],
                        long = gtfs_list_by_mode$stops$stop_lon[i])
   ...
   # Buffer circles by the buffer distance
    ...
    SL_Bn <- arrival_seven_day_function(gtfs_list_by_mode = gtfs_list_by_mode, stop_to_show = gtfs_list_by_mode$stops$stop_name[i], first_date = start_date)
